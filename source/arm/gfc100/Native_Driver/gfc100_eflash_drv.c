                                    ...
                                    uint32_t offset)
{
    ...
    /* Maximum 16 bytes can be copied in one block */
    size = (size > GFC100_READ_BYTE_SIZE) ? GFC100_READ_BYTE_SIZE : size;
    /* We might not copy from Data reg0, adjust size with offset if needed*/
